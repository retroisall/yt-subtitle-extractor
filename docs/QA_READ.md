...
#### 4. 只量 getBoundingClientRect().width 認為版面正確
```js
// 這樣測不到視覺遮蓋問題
const w = await page.evaluate(() => document.querySelector('#primary').getBoundingClientRect().width);
assert(w > 800, '#primary 夠寬');
```
`#primary` 的寬度可能完全正確（如 889px），但有另一個元素（如 `#panels-full-bleed-container`）
疊在上方，造成使用者看到黑色遮罩。光量寬度無法偵測「視覺上是否被遮蓋」。

**正確做法**：同時驗證可能造成遮蓋的兄弟元素的 `display` 狀態：
```js
...
assert(panelsDisplay === 'none' || panelsDisplay === 'not-found', '無遮罩元素');
```
...
- [ ] 版面測試同時驗證「可能視覺遮蓋的元素」的 display 狀態，不只量 getBoundingClientRect().width
